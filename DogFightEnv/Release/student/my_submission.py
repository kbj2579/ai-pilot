...
#   - 인터넷 너머 원격: 호스트의 공인 IP + 포트포워딩 필요
# 대회 서버 IP는 "221.151.77.208" (운영진 공지 기준, 실제 대회 제출 때만 사용)
SERVER_IP = "127.0.0.1"                            # TODO: 스크림 상대의 서버 IP로 교체
SERVER_PORT = 9999

# 사용할 백엔드 모드 선택: "rl" | "bt" | "hybrid"
# 아직 완성된 학습 모델이 없으므로 BT 모드로 시작 (연결/네트워크 확인용).
# 모델이 준비되면 "rl"로 바꾸고 BUNDLE_DIR을 실제 경로로 채우세요.
MODE = "bt"

# RL 모드 설정 (MODE="rl" 또는 "hybrid"일 때 사용)
...
# BT 모드 설정
# - Rule_forTraining.xml은 저장소 루트의 Rule.xml을 복사해 Release/ 루트에
#   만들어 둔 최소 동작 버전입니다(직진만 하는 스텁 — 스크림 연결 테스트용).
...
# 연결 설정
AI_TYPE = AIType.RuleBased if MODE == "bt" else AIType.ReinforcementLearning
HEARTBEAT_SEC = 1.0
